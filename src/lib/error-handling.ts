...
export const err = {
	handler<E extends (Record<any, any> | null)[]>(...args: E) {
		...
		return <
			K extends Readonly<StatusCodeFn['Error']>,
			T extends Parameters<typeof HTTP[K]>[0] = {}
		>(
			response: K,
			options?: T,
			message?: string
			...
			return HTTP[response](o) as CreateResponse<
				K,
				StatusText[K],
				false,
				typeof o & T
	...
	require<T extends Record<any, any>>(
		obj: T,
	...
	test<K extends Record<any, string>>(
		condition: any,
		obj: K
	): Partial<K> | null {
	...
	type<T extends Record<any, any>>(
		obj: T,
		typing: { [K in keyof T]: Type } | Type
	): { [P in keyof T]?: string } | null {
		let errors: Record<any, any> = {}
		if (typeof typing === 'string') {
			...
			for (let [key, type] of Object.entries(typing)) {
				if (typeof obj[key] !== type) errors[key] = `Is not a '${type}'`
			}
		}
		return Object.keys(errors).length > 0 ? (errors as any) : null
	},
	/** Ignores undefined/null values — if value(s) does not match, will result in error. */
	match<T extends Record<any, any>>(
		obj: T,
		regex: Record<keyof T, RegExp> | RegExp,
		errorMsg: string
	): { [P in keyof T]?: string } | null {
		let errors: Record<any, any> = {}
